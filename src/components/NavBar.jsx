...
const NavBar = () => {
	...
	return (
		<div className={`${agentDetailById?.color?.textColor || null} navbar w-full flex items-start justify-between`}>
			<div>
			...
			<nav>
				<ul className="flex space-x-23 items-center leading-none uppercase">
					...
					<button className="flex flex-col space-y-1  ">
						<span className={`${agentDetailById?.color?.bgColor || null} h-1 w-7`}></span>
						<span className={`${agentDetailById?.color?.bgColor || null} h-1 w-4`}></span>
					</button>
				</ul>
			</nav>
		</div>
